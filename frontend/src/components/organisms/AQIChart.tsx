...
const AQIChart: React.FC<AQIChartProps> = ({
  ...
  const chartData = useMemo(() => {
    ...
    const imputedSegments: Array<Array<[string, number | null]>> = []
    let i = 0
    while (i < allData.length) {
      if (allData[i].isImputed && allData[i].value !== null) {
        const segment: Array<[string, number | null]> = []
    ...
    const imputedLineData: Array<[string | null, number | null]> = []
