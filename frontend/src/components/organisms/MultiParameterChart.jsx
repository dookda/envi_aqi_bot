...
export default function MultiParameterChart({
    ...
    const [internalSelectedParam, setInternalSelectedParam] = useState('pm25')
    ...
    const chartData = useMemo(() => {
        ...
        const thresholds = SPIKE_THRESHOLDS[selectedParam]
        const imputedField = `${selectedParam}_imputed`
        ...
        const allValues = sortedData.map(d => d[selectedParam])
        ...
        sortedData.forEach((d, i) => {
            ...
            const value = d[selectedParam]
            const isImputed = d[imputedField] || d.is_imputed || false
    ...
    useEffect(() => {
        ...
        const option = {
            ...
            yAxis: {
                ...
                min: dataMin => Math.max(0, dataMin - 5),
            },
            dataZoom: [
                    ...
                    <div className="flex items-center gap-4">
                        <label className={`flex items-center gap-2 cursor-pointer ${isLight ? 'text-gray-600' : 'text-dark-300'}`}>
                            <input
                                type="checkbox"
                                checked={showGapFill}
                                onChange={(e) => setShowGapFill(e.target.checked)}
