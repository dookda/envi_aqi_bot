...
export default function DataTable({
    ...
    const [sortConfig, setSortConfig] = useState({ key: null, direction: 'desc' }) // Default desc for time series

    // Reset page when data changes
    if (currentPage > Math.ceil((data?.length || 0) / pageSize) && data?.length > 0) {
        setCurrentPage(1)
    }
                        ...
                        <thead className={`${isLight ? 'bg-gray-50' : 'bg-dark-800'}`}>
                            <tr>
                                {columns.map((col, index) => (
                                    <th
                                        key={index}
                                        className={`px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider cursor-pointer select-none transition-colors ${isLight ? 'text-gray-500 hover:bg-gray-100' : 'text-dark-300 hover:bg-dark-700'
                                            } ${col.className || ''}`}
                                ...
                                paginatedData.map((row, rowIndex) => (
                                    ...
                                    >
                                        {columns.map((col, colIndex) => (
                                            <td
                                                key={colIndex}
                                                className={`px-6 py-4 text-sm whitespace-nowrap ${isLight ? 'text-gray-700' : 'text-gray-300'
                                                    } ${col.className || ''}`}
