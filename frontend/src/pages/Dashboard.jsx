...
import { Button, Card, Icon, Badge, Spinner } from '../components/atoms'
import { StatCard, StationSelector, DataTable } from '../components/molecules'
import { AQIChart, StationMap, Navbar, MultiParameterChart } from '../components/organisms'
...
const ParameterGauge = ({ label, value, unit, icon, thresholds, maxScale, isLight }) => {
    const percentage = value ? Math.min((value / maxScale) * 100, 100) : 0

    // Find current threshold level
    const currentLevel = thresholds.find(t => value <= t.max) || thresholds[thresholds.length - 1]
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (row) => {
                ...
                const isImputed = row.is_imputed || row[`${selectedParam}_imputed`]
                return (
...
export default function Dashboard() {
    ...
    const currentStation = stations.find(s => s.station_id === selectedStation)
    const latestData = fullData?.data?.[0] || {}
    const currentPm25 = latestData.pm25 || stats.mean
            ...
            <main className="max-w-7xl mx-auto px-4 py-6">
                ...
                <section className="mb-6">
                    <div className={`flex gap-2 p-1 rounded-xl ${isLight ? 'bg-gray-100' : 'bg-dark-800'}`}>
                        {[
                            { id: 'overview', label: lang === 'th' ? 'ภาพรวม' : 'Overview', icon: 'dashboard' },
                            { id: 'charts', label: lang === 'th' ? 'กราฟ' : 'Charts', icon: 'show_chart' },
                            { id: 'map', label: lang === 'th' ? 'ข้อมูล' : 'Data', icon: 'table_chart' },
                        ].map(tab => (
                            <button
                                key={tab.id}
                    ...
                    <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                        ...
                        <Card className="p-6">
                            ...
                            <div className="space-y-3">
                                {Object.entries(AQI_LEVELS).map(([key, level]) => (
                                    <div
                                        key={key}
