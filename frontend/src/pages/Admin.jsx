...
export default function Admin() {
    ...
    const fetchDataStatus = async (isAutoRefresh = false) => {
        try {
            if (!isAutoRefresh) {
                setRefreshing(true)
            }
            const [status, logs, scheduler] = await Promise.all([
                api.get('/admin/data-status?sample_size=10'),
                api.get('/ingest/logs?limit=10'),
    ...
    const lastIngestion = dataStatus?.last_ingestion || {}
            ...
            <main className="max-w-7xl mx-auto px-4 py-6">{loading ? (
                    ...
                    <div className="border-b border-gray-700">
                        <nav className="-mb-px flex space-x-8 overflow-x-auto">
                            {[
                                ...
                                { id: 'comparison', label: lang === 'th' ? 'เปรียบเทียบข้อมูล' : 'Data Comparison', icon: 'compare_arrows' },
                                { id: 'scheduler', label: lang === 'th' ? 'ตัวจัดกำหนดการ' : 'Scheduler', icon: 'event_repeat' },
                                { id: 'logs', label: lang === 'th' ? 'ประวัติการดึงข้อมูล' : 'Ingestion Logs', icon: 'history' }
                            ].map(tab => (
                                <button
                                    key={tab.id}
                ...
                {activeTab === 'overview' && (
                    ...
                    <StatCard
                        ...
                        color={summary.sync_rate_percentage >= 80 ? 'success' : 'warning'}
                        iconName="percent"
                    />
                    <StatCard
